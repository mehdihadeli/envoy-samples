...
The Makefile builds the example server with `make example` and sample `example` PHONY name in Makefile and then runs `build/example.sh` which runs both `Envoy` and the `example server`.it create a grpc server on this ip `127.0.0.1:18000` in our envoy config. The example server serves a configuration defined in `internal/example/resource.go`.  If everything works correctly, you should be able to open a browser to [http://localhost:10000](http://localhost:10000) and see Envoy's website.
